...
const ModalBox = createReactClass({
  ...
  createPanResponder() {
    ...
    const onPanRelease = (evt, {dy}) => {
      if (!inSwipeArea) return;
      inSwipeArea = false;
      if (this.props.entry === 'top' ? -dy > this.props.swipeThreshold : dy > this.props.swipeThreshold)
        this.animateClose();
      else
        this.animateOpen();
    };
    ...
    const onPanStart = ({nativeEvent}, state) => {
      if (!this.props.swipeToClose || this.props.isDisabled || (this.props.swipeArea && (nativeEvent.pageY - this.state.positionDest) > this.props.swipeArea)) {
        inSwipeArea = false;
        return false;
      }
      inSwipeArea = true;
